...
class AdminService:
    ...
    __banner_service = BannerService()
    
    def link_telegram(self, code: str) -> str:
        chat_id = self.r.get(code)

        if chat_id is None:
            raise NotFoundException
        try:
            self.rq.publish(action="new_session", message=f"{chat_id}")
        except Exception as e:
            print("Ошибка отправки сообщения с chat_id")
        finally:
            self.rq.close()
    ...
    def upload_banner(self, serializer: BannerSerializer, group: Banner.BannerGroupType):
        max_queue = self.__banner_service.get_max_queue(group)
    ...
    def get_admin_queryset(self, group: Banner.BannerGroupType) -> Iterable[Banner]:
        return self.__banner_service.get_admin_queryset(group)