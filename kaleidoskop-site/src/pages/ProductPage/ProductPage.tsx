...
import {
  fetchProductBySlug,
  toggleWishlist,
} from "../../features/products/productItemSlice";
...
function ProductPage() {
  ...
  useEffect(() => {
    if (categorySlug) {
      dispatch(fetchCategoryBySlug(categorySlug)).then((res) => {
  ...
  useEffect(() => {
    if (category?.parent_slug) {
      dispatch(fetchCategoryBySlug(category.parent_slug)).then((res) => {
        setParentCategory(res.payload);
      });
    } else {
  ...
  useEffect(() => {
    if (selectedItem && selectedItem.slug !== prevProductIdRef.current) {
      setGalleryKey((prev) => prev + 1);
      prevProductIdRef.current = selectedItem.slug;
    }
  }, [selectedItem]);
  ...
  const handleToggleWishlist = (e: React.MouseEvent) => {
    if (selectedItem) {
      ...
      dispatch(
        toggleWishlist({
          id: selectedItem.id,
          enable: !selectedItem.in_wishlist,
        }),
      );
    }
  };
  ...
  return (
    <div className="page-product">
      ...
      <div className="page-path inter16-400">
        ...
        {categorySlug && (
          <>
            ...
            <Link to={`/category/${categorySlug}`} className="main-link">
              {subCategory?.title || categorySlug}
            </Link>
            <span className="page-path_separator">/</span>
          </>
        )}

        <span className="page-path_name">{selectedItem?.title}</span>
      </div>
        ...
        <div className="product-info_main">
          ...
          <div className="product-info_actions">
            <ToBasket
              ...
              onAdd={() =>
                dispatch(
                  toggleBasketItem({ id: selectedItem.id, enable: true }),
                )
              }
              onRemove={() =>
                dispatch(
                  toggleBasketItem({ id: selectedItem.id, enable: false }),
                )
              }
              onIncrease={() =>
                dispatch(
      ...
      <div className="product-recs">
        <ItemsBlock title="Похожие товары" items={recommendations} />
      </div>
    </div>
  );
}
