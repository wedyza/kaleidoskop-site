...
const NomenclaturesModal: React.FC<NomenclaturesModalProps> = ({
  isModalOpen,
  setIsModalOpen,
  selectedNom,
}) => {
  const { categories } = useAppSelector((state) => state.adminCategories);
  const { addToCategoryLoading, deleteFromCategoryLoading } = useAppSelector(
    (state) => state.nomenclatures,
  );
  ...
  const isCategorySelected = (categoryId: string) => {
    return (
      selectedNom.categories?.some((cat) => cat.id === categoryId) || false
    );
  };
  ...
  return (
    <Modal
      ...
      className="admin-nom_modal"
    >
      <h3 className="admin-nom_modal-title inter14-400">
        Назначьте подкатегорию для
        <span className="inter14-700">{selectedNom.title}</span>
      </h3>
        ...
        {filteredCategories.map((cat) => {
          ...
          return (
            <div className="admin-nom_cat" key={cat.id}>
              <span className="admin-nom_cat-title inter14-400">
                {cat.title}
              </span>
              ...
              {isSelected ? (
                <div className="admin-nom_cat-selected">
                  <div className="admin-nom_cat-selected-label">
                    ...
                    <span className="admin-nom_cat-selected-text inter14-600">
                      Назначена
                    </span>
                  </div>
                  ...
                  >
                    ...
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0.220236 0.219644C0.513129 -0.0732486 0.988003 -0.0732489 1.2809 0.219644L5.99321 4.93195L10.7055 0.219644C10.9984 -0.0732486 11.4733 -0.0732489 11.7662 0.219644C12.0591 0.512537 12.0591 0.987412 11.7662 1.2803L7.05387 5.99261L11.7662 10.7049C12.0591 10.9978 12.0591 11.4727 11.7662 11.7656C11.4733 12.0585 10.9984 12.0585 10.7055 11.7656L5.99321 7.05327L1.2809 11.7656C0.988003 12.0585 0.513129 12.0585 0.220236 11.7656C-0.0726573 11.4727 -0.0726572 10.9978 0.220236 10.7049L4.93255 5.99261L0.220236 1.2803C-0.0726572 0.987411 -0.0726573 0.512538 0.220236 0.219644Z"
                        fill="#727271"
                      />
                    </svg>
                  </button>
